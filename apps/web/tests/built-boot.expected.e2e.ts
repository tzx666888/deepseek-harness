...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('DSH_CLIENT_BUILD_PROFILE') === 'official') {
    ...
    expect(screen.queryByText('鑫哥专属')).toBeNull()
  } else {
    expect(document.querySelector('[data-local-brand-mark="planet"]')).not.toBeNull()
    screen.getByText('Xinge Exclusive')
    expect(document.querySelector('.buildVersion')).toBeNull()
  }
