...
/** Public client environment required by the personalized Xinge desktop build. */
const XINGE_CLIENT_BUILD_ENVIRONMENT = {
...
/** Resolve the public identity expected in the personalized Xinge desktop artifacts. */
export function xingeClientBuildEnvironment(
  root: string,
  environment: NodeJS.ProcessEnv = process.env,
): Readonly<Record<`DSH_CLIENT_${string}`, string>> {
  return {
    DSH_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    DSH_CLIENT_VERSION: repositoryVersion(root),
    ...XINGE_CLIENT_BUILD_ENVIRONMENT,
  }
}
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official' || profile === 'xinge') {
    ...
    return {
      ...
      ...(profile === 'official' ? OFFICIAL_CLIENT_BUILD_ENVIRONMENT : XINGE_CLIENT_BUILD_ENVIRONMENT),
    }
  }
  throw new Error(`unknown client build profile ${JSON.stringify(profile)}; expected "official" or "xinge"`)
}
