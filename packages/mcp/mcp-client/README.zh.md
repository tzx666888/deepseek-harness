...
| `restartOnInterruptedCall` | `false` | 仅 stdio：调用超时或取消后替换所拥有的服务器；不自动重放该调用 |
| `allowTools` | `[]` | 可选的原始 MCP 工具名称白名单。空列表会暴露全部工具；使用非空列表可避免向模型目录暴露敏感或无关的服务器工具。 |
...
对于忽略取消请求的本地服务器，启用 `restartOnInterruptedCall`。调用中断后会关闭所拥有的进程，等待其退出，并从替换进程刷新工具后再返回。并发中断共享一次替换。这会重置共享该服务器的所有会话，且可能丢弃服务器内部状态；已经完成的外部效果不会撤销。普通工具错误不会触发重启。重试修改操作前先检查目标状态。浏览器扩展用户需要批准新的连接；超时不能证明用户拒绝了授权。
