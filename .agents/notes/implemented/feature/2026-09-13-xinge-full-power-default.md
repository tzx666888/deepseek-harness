...
On macOS, the desktop shell discovers a locally installed Peekaboo executable or accepts its absolute path through `DSH_CONTROL_COMMAND`. Xinge Full Power conditionally starts that MCP server inside the agent preset and exposes an explicit allowlist of screen observation, application, window, pointer, and keyboard tools under the `mcp__desktop__` namespace. Raw clipboard reading and the server's autonomous agent surface are not registered. A shipped browser-control skill requires both Screen Recording and Accessibility grants before mutation, forbids blind coordinate or shortcut operation, and verifies results from the visible screen instead of copying user content.
...
**Register every tool advertised by the desktop server.** Rejected because raw clipboard reads can disclose unrelated private data and a second autonomous agent surface duplicates the Harness loop. The MCP client's raw-name allowlist keeps those capabilities out of the model catalog and reduces prompt overhead.
...
New desktop and browser sessions without a stored override start in Xinge Full Power and can use persistent interactive programs, structured file replacement, native delegation, Codex delegation, workflows, and runtime extension immediately. On a macOS installation with Peekaboo and both required operating-system grants, those sessions can also inspect and operate visible desktop applications. Missing or unknown grants stop the operation rather than falling back to blind control, and the model cannot directly read the user's clipboard. The optional Codex runtime and MCP client become part of the installation closure, increasing packaged size. External account-backed capabilities still appear only after their provider or MCP connection is configured. Existing sessions keep the preset they were created with, so users must start a new session to receive the new default.
